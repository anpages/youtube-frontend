import { isAuthenticated, isSessionExpired, signIn } from '../auth.js'
...
export function renderHome() {
  ...
  // If the session expired (had session before, but silent refresh failed),
  // show a targeted reconnect screen instead of the full marketing landing page.
  if (isSessionExpired()) {
    const app = document.getElementById('app')
    app.innerHTML = `
      <div class="min-h-[80vh] flex flex-col items-center justify-center px-4 text-center gap-6">
        <svg class="w-14 h-14 text-neutral-600" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" aria-hidden="true">
          <path stroke-linecap="round" stroke-linejoin="round" d="M16.5 10.5V6.75a4.5 4.5 0 10-9 0v3.75m-.75 11.25h10.5a2.25 2.25 0 002.25-2.25v-6.75a2.25 2.25 0 00-2.25-2.25H6.75a2.25 2.25 0 00-2.25 2.25v6.75a2.25 2.25 0 002.25 2.25z"/>
        </svg>
        <div>
          <h2 class="text-xl font-semibold text-neutral-200 mb-1">Tu sesión ha expirado</h2>
          <p class="text-neutral-500 text-sm">Vuelve a conectar tu cuenta de Google para continuar.</p>
        </div>
        <button
          id="reconnect-btn"
    ...
    `
    document.getElementById('reconnect-btn').addEventListener('click', signIn)
